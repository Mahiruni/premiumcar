...
export default function Page() {
  ...
  useEffect(() => {
    ...
    async function load() {
      if (!supabase) {
        if (mounted) { setMessage('Marketplace database is not configured.'); setLoadingCategories(false) }
        return
      }
      try {
        const [{ data: sessionData }, categoriesResult] = await Promise.all([
          supabase.auth.getSession(),
          withTimeout(supabase.from('categories').select('id,name').order('name'), 'Categories could not be loaded. Please refresh and try again.'),
        ])
        if (!mounted) return
        ...
        if (mounted) setMessage(error instanceof Error ? error.message : 'Could not load the sell page.')
      } finally {
        if (mounted) setLoadingCategories(false)
      }
    }
    load()
    const listener = supabase?.auth.onAuthStateChange((_event, nextSession) => { if (mounted) setSession(nextSession) })
  ...
  async function publish(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!supabase) { setMessage('Marketplace database is not configured.'); return }
    setPublishing(true)
    ...
    try {
      ...
      if (!title || !description || !city || !categoryId || !Number.isFinite(price) || price < 0) throw new Error('Please complete all required listing details.')

      // IMPORTANT: create the listing BEFORE processing/uploading photos. A slow phone/browser image operation must never block ad creation.
      setMessage('Creating your ad…')
      const listingResult = await withTimeout(
        supabase.from('listings').insert({ seller_id: user.id, category_id: categoryId, title, description, price, city, condition }).select('id').single(),
        'The marketplace database did not respond. Please try again.'
      )
      if (listingResult.error || !listingResult.data) throw new Error(listingResult.error?.message || 'Could not create the listing.')
      ...
      setMessageType('success')
      setMessage('Ad created. Optimizing and uploading photos in the background…')

      const results = await Promise.all(files.map(async (originalFile, index) => {
        try {
          const file = await compressAndWatermark(originalFile, uploaderName)
          const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, '-').slice(-120)
          const path = `${user.id}/${listingId}/${crypto.randomUUID()}-${safeName}`
          const uploadResult = await withTimeout(
            supabase.storage.from('listing-images').upload(path, file, { contentType: 'image/webp', upsert: false }),
            `Photo ${index + 1} upload timed out.`
          )
          if (uploadResult.error) return { ok: false, error: uploadResult.error.message }
          const imageRow = await withTimeout(
            supabase.from('listing_images').insert({ listing_id: listingId, storage_path: path, sort_order: index }),
            `Photo ${index + 1} could not be attached.`
          )
          return imageRow.error ? { ok: false, error: imageRow.error.message } : { ok: true }
        } catch (error) {
          return { ok: false, error: error instanceof Error ? error.message : 'Photo failed' }
        }
      }))

      const uploaded = results.filter((r) => r.ok).length
      const failed = results.length - uploaded
      setMessage(`Your ad was published successfully with ${uploaded} optimized, watermarked photo(s).${failed ? ` ${failed} photo(s) could not be uploaded, but the ad is already published.` : ''}`)
      e.currentTarget.reset()
      setFiles([])
    } catch (error) {
      setMessageType('error')
